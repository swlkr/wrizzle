...
RESET_BUTTON='<button hx-post="/reset" hx-target="#guesses" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Reset</button>'

GUESS_FORM="
<input type=\"text\" name=guess id=guess placeholder=\"guess here\" class=\"uppercase\" autocomplete=\"off\"></input>
<button _=\"on htmx:afterRequest get #guess then put '' into its value\" type=\"submit\" hx-post=\"/guess\" hx-target=\"#guesses\" class=\"bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded\">Guess</button>
"
